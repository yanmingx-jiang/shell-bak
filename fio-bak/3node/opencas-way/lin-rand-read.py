...
# import sh
# import codecs
rmrf = subprocess.Popen("ssh root@192.168.10.2 'rm -rf *.fio' && ssh root@192.168.10.3 'rm -rf *.fio' && ssh root@192.168.10.4 'rm -rf *.fio' && ssh root@192.168.10.4 'rm -rf *.txt'", shell=True, stdout=subprocess.PIPE)
...
node1 = subprocess.Popen("ssh root@192.168.10.2 'fio rand-read-libaio.fio' > out/node1-rand-read.fio.out 2>&1 &", shell=True, stdout=subprocess.PIPE)

node2 = subprocess.Popen("ssh root@192.168.10.3 'fio rand-read-libaio.fio' > out/node2-rand-read.fio.out 2>&1 &", shell=True, stdout=subprocess.PIPE)

# node3 = subprocess.Popen("ssh root@192.168.10.4 'fio rand-read-libaio.fio' > out/node3-rand-read.fio.out 2>&1 &", shell=True, stdout=subprocess.PIPE)

dstatout= subprocess.Popen("ssh root@192.168.10.2 'dstat -tcmynd -D total,sda,sdb,sdc,sdd,sde,sdf,sdg,nvme0n1 | tee ~/rand-read-dstat.txt 2>&1 &'", shell=True, stdout=subprocess.PIPE)
...
scp = subprocess.Popen("scp root@192.168.10.2:~/rand-read-dstat.txt /home/workspace/ceph/3node/test0304/out", shell=True, stdout=subprocess.PIPE)
